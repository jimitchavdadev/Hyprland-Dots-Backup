...
Item {
    ...
    readonly property int workspaceGroup: monitor && monitor.activeWorkspace ? Math.floor((monitor.activeWorkspace.id - 1) / workspacesShown) : 0
    property bool monitorIsFocused: monitor && Hyprland.focusedMonitor ? (Hyprland.focusedMonitor.name == monitor.name) : false
    property var windows: HyprlandData.windowList
    property var windowByAddress: HyprlandData.windowByAddress
    property var windowAddresses: HyprlandData.addresses
    property var monitorData: monitor ? HyprlandData.monitors.find(m => m.id === monitor.id) : null
    property real scale: Config.options.overview.scale
    property color activeBorderColor: Appearance.colors.colSecondary

    property real workspaceImplicitWidth: {
        var w = monitor ? monitor.width : 1920;
        var h = monitor ? monitor.height : 1080;
        var s = monitor ? monitor.scale : 1.0;
        if (s <= 0) s = 1.0;
        var r0 = monitorData && monitorData.reserved ? (monitorData.reserved[0] ?? 0) : 0;
        var r2 = monitorData && monitorData.reserved ? (monitorData.reserved[2] ?? 0) : 0;
        if (monitorData && monitorData.transform % 2 === 1) {
            return ((h / s - r0 - r2) * root.scale);
        } else {
            return ((w / s - r0 - r2) * root.scale);
        }
    }
    property real workspaceImplicitHeight: {
        ...
        var s = monitor ? monitor.scale : 1.0;
        if (s <= 0) s = 1.0;
        var r1 = monitorData && monitorData.reserved ? (monitorData.reserved[1] ?? 0) : 0;
        var r3 = monitorData && monitorData.reserved ? (monitorData.reserved[3] ?? 0) : 0;
        if (monitorData && monitorData.transform % 2 === 1) {
            return ((w / s - r1 - r3) * root.scale);
        } else {
            return ((h / s - r1 - r3) * root.scale);
        }
    }

    property real workspaceNumberMargin: 80
    property real workspaceNumberSize: 250 * (monitor ? monitor.scale : 1.0)
